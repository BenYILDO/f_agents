...
class TestFetchTurkishMacroNews:
    def test_buckets_macro_and_drops_noise(self):
        ...
        with patch.object(turkish_macro, "_fetch_feed", return_value=items):
            res = turkish_macro.fetch_turkish_macro_news(
        ...
        # health: one feed, returned items -> ok
        assert res.ok and len(res.sources) == 1 and res.sources[0].is_ok
    ...
    def test_dedupes_across_feeds(self):
        ...
        assert res.text.count("TCMB faizi sabit tuttu") == 1

    def test_all_feeds_down_returns_placeholder_and_unhealthy(self):
        with patch.object(turkish_macro, "_fetch_feed", return_value=[]):
            res = turkish_macro.fetch_turkish_macro_news(
                feeds=(("S", "http://x"),), inter_request_delay=0,
            )
        assert "ulaşılamadı" in res.text
        assert res.ok is False and res.sources[0].status == "empty"
